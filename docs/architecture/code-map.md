...
| Eval-run provenance parsing and envelope validation | Planned: `openevalgate/provenance.py` | Contract fixtures: `tests/test_provenance_contract_fixtures.py`; planned runtime tests: `tests/test_provenance.py`, `tests/test_provenance_fixtures.py` | Contract-development validation covers schemas, fixture inventory, referenced files, digests, orphan-file hygiene, and selected scenario invariants; runtime parser and complete classifier ownership remains planned. |
| Provenance freshness and release-context comparison | Planned: `openevalgate/provenance.py` | Contract fixtures: `tests/test_provenance_contract_fixtures.py`; planned runtime tests: `tests/test_provenance_freshness.py` | Contract fixtures define normative expected freshness, recency, and authorization classifications; PR 18 checks selected invariants only, while runtime freshness comparison and authorization evaluation remain planned. |
