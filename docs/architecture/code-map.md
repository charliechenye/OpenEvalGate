...
| --- | --- | --- | --- |
| CLI entry point and exit behavior | `openevalgate/cli.py` | `tests/test_report.py`, `tests/test_project_inspection.py`, `tests/test_version.py` | Console script is declared in `pyproject.toml`; exit codes and stdout/stderr are compatibility-sensitive. |
...
| Eval-run provenance parsing and identity validation | `openevalgate/provenance.py`, `openevalgate/local_paths.py` | `tests/test_provenance.py`, `tests/test_provenance_fixtures.py`, `tests/test_provenance_contract_fixtures.py`, `tests/test_eval_results.py` | Owns packaged schema loading, deterministic root/scoped manifest selection, selected run/candidate identity, lifecycle findings, authoritative result source selection, output identity, artifact-index identity, local digest verification, historical envelope consistency, scope-aware conventional unbound-result detection, and fail-closed identity invalidation. Freshness, recency, and review-context comparison remain deferred. |
| Provenance freshness and release-context comparison | Planned extension to `openevalgate/provenance.py` | Contract fixtures: `tests/test_provenance_contract_fixtures.py`; future runtime tests: `tests/test_provenance_freshness.py` | Contract fixtures define normative expected freshness, recency, verified assurance, and authorization classifications. Runtime identity enforcement, local digest verification, and historical envelope consistency are implemented, but `review_context.yaml`, freshness, and recency comparison remain deferred. |
