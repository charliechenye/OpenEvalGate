...
| Eval-run provenance parsing and identity validation | `openevalgate/provenance.py`, `openevalgate/local_paths.py` | `tests/test_provenance.py`, `tests/test_provenance_fixtures.py`, `tests/test_provenance_contract_fixtures.py`, `tests/test_eval_results.py` | Owns packaged schema loading, deterministic manifest selection, selected run/candidate identity, lifecycle findings, authoritative result source selection, output identity, artifact-index identity, legacy distinction, and fail-closed identity invalidation. Digest verification, freshness, recency, and review-context comparison remain deferred. |
