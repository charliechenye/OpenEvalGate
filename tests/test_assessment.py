...
def _sufficiency(
    ...
    *,
    ...
    return BehavioralSufficiency(
        state,
        mode if policy_present and policy_valid else None,
        mode,
        policy_present,
        policy_valid,
        "run" if selected_rows else "run",
        "candidate" if selected_rows else "candidate",
        selected_rows,
        1,
        1 if selected_rows else 0,
        1.0 if selected_rows else 0.0,
        1 if selected_rows else 0,
        () if selected_rows else ("case",),
        (),
        0,
        0,
        None,
...
def _assess(
    ...
    *,
    score: int = 100,
    valid: bool = True,
    blockers: list[HardBlocker] | None = None,
):
...
def test_invalid_policy_has_no_shadow_fallback() -> None:
    invalid = replace(
        ...
        declared_mode=None,
        effective_mode=None,
        sufficient_for_requested_mode=False,
    )
...
def test_controlled_mode_requires_valid_results(state: str) -> None:
    result = _assess(
        _sufficiency(ReviewMode.CONTROLLED_LAUNCH, state=state, selected_rows=0, sufficient=False)
    )
    assert result.maximum_permitted_stage == "Shadow evaluation"
...
def test_controlled_mode_zero_scope_rows_fails_closed() -> None:
    ...
    assert (
        "Provide matching result rows for the selected run and candidate."
        in result.recommended_next_actions
    )


def test_controlled_mode_failure_and_hard_blocker_precedence() -> None:
    failed = _sufficiency(ReviewMode.CONTROLLED_LAUNCH, sufficient=False, invariant_status="fail")
...
@pytest.mark.parametrize(
    ("score", "expected"),
    [
        (100, "Substantially complete"),
        (85, "Substantially complete"),
        (84, "Material gaps"),
        (50, "Material gaps"),
        (49, "Incomplete"),
    ],
